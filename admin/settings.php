...
namespace ASI_WPGraphQL\Admin\Settings;
...
function add_admin_menu_pages()
{
    add_submenu_page(
        'options-general.php',
        'React WPGraphql Plugin Settings',
        'React WPGraphql Settings  <span class="dashicons dashicons-admin-generic"></span>',
        is_admin(),
...
function init_custom_settings()
{
    ...
    add_settings_section(
        'react-wpgraphql-plugin-features-section',
        'Toggle Features',
        __NAMESPACE__ . '\render_features_section',
        '' . PLUGIN_PREFIX . '_do_settings_section'
    );
    add_settings_section(
        'react-wpgraphql-plugin-recaptcha-section',
        'reCAPTCHA Settings ',
    ...
    add_settings_field(
        'react-wpgraphql-plugin-allow-portal-registration',
    ...
    //Recaptcha Fields
    add_settings_field(
        'react-wpgraphql-plugin-recaptcha-site-key',
        'reCAPTCHA site key ',
    ...
    add_settings_field(
        'react-wpgraphql-plugin-recaptcha-secret-key',
    ...
    //Features
    register_setting('react-wpgraphql-plugin-settings', 'allow_portal_registration');

    //reCaptcha
    register_setting('react-wpgraphql-plugin-settings', '' . PLUGIN_PREFIX . '_recaptcha_site_key');
    register_setting('react-wpgraphql-plugin-settings', '' . PLUGIN_PREFIX . '_recaptcha_secret_key');
}
//Features Section
function render_features_section()
{
    echo '<h4><span role="img" aria-label="features section">⚙️</span> Enable/disable features</h4>';
}
...
{
    require_once ASI_WPGRAPHQL_PLUGIN_DIR . 'admin/admin-pages.php';
}
